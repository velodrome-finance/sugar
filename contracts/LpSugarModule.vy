...
# @title Velodrome Finance LP Sugar v3 Module
...
def _positions(
  ...
  if _alm_factory != empty(IAlmFactory):
    alm_core = IAlmCore(_alm_factory.getImmutableParams()[0])
...
def _cl_position(
  ...
  if _gauge == empty(address):
    gauge = ICLGauge(_voter.gauges(pos.lp))

  amounts: Amounts = cl_helper.principal(
  ...
  pos.sqrt_ratio_lower = _cl_helper.getSqrtRatioAtTick(pos.tick_lower)
  pos.sqrt_ratio_upper = _cl_helper.getSqrtRatioAtTick(pos.tick_upper)

  amounts_fees: Amounts = _cl_helper.fees(nfpm.address, pos.id)
...
def _v2_position(_account: address, _pool: address, _voter: IVoter) -> Position:
  ...
  gauge: IGauge = IGauge(_voter.gauges(_pool))
  decimals: uint8 = pool.decimals()
...
@internal
@view
def _cl_lp(_data: address[4], _token0: address, _token1: address, _voter: IVoter, _cl_helper: ISlipstreamHelper, _alm_factory: IAlmFactory) -> Lp:
  """
  ...
  gauge_alive: bool = _voter.isAlive(gauge.address)
  ...
  if gauge_liquidity > 0 and gauge.address != empty(address):
    ...
    ratio_a: uint160 = _cl_helper.getSqrtRatioAtTick(tick_low)
    ratio_b: uint160 = _cl_helper.getSqrtRatioAtTick(tick_high)
    staked_amounts: Amounts = _cl_helper.getAmountsForLiquidity(
  ...
  if _alm_factory != empty(IAlmFactory):
    alm_addresses = _alm_factory.poolToAddresses(pool.address)
...
def _poolRewards(_venft_id: uint256, _pool: address, _gauge: address, _voter: IVoter) \
  ...
  fee: IReward = IReward(_voter.gaugeToFees(_gauge))
