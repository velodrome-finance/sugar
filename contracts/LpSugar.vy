...
# from "./LpSugarModule.vy" import LpSugarModule as helper
...
def forSwaps(_limit: uint256, _offset: uint256) -> DynArray[SwapLp, MAX_POOLS]:
  ...
  for index in range(0, MAX_FACTORIES):
    ...
    nfpm: address = helper._fetch_nfpm(factory.address)
...
def tokens(_limit: uint256, _offset: uint256, _account: address, \
  ...
  """
  pools: DynArray[address[4], MAX_POOLS] = helper._pools(_limit, _offset)
  ...
  for index in range(0, MAX_TOKENS):
    ...
    col.append(helper._token(_addresses[index], _account))
  ...
  for index in range(0, MAX_POOLS):
    ...
    if token0 not in seen:
      col.append(helper._token(token0, _account))
      seen.append(token0)

    if token1 not in seen:
      col.append(helper._token(token1, _account))
      seen.append(token1)
...
def all(_limit: uint256, _offset: uint256) -> DynArray[Lp, MAX_LPS]:
  ...
  for index in range(0, MAX_POOLS):
    ...
    if pool_data[3] != empty(address):
      col.append(helper._cl_lp(pool_data, token0, token1))
    else:
      col.append(helper._v2_lp(pool_data, token0, token1))
...
def byIndex(_index: uint256) -> Lp:
  ...
  pool_data: address[4] = helper._pools(1, _index)[0]
  pool: IPool = IPool(pool_data[1])
  token0: address = pool.token0()
  token1: address = pool.token1()
  ...
  # If this is a CL factory/NFPM present...
  if pool_data[3] != empty(address):
    return helper._cl_lp(pool_data, token0, token1)

  return helper._v2_lp(pool_data, token0, token1)
...
def positionsByFactory(
  ...
  @return Array for Lp structs
  """
  return helper._positions(_limit, _offset, _account, [_factory])
...
def unstakedPositions(
  ...
  for index in range(0, MAX_FACTORIES):
    ...
    nfpm: INFPositionManager = \
      INFPositionManager(helper._fetch_nfpm(factory.address))
    ...
    for pindex in range(0, MAX_POSITIONS):
      ...
      pos: Position = helper._cl_position(
        pos_id,
        _account,
        empty(address),
        empty(address),
...
def epochsLatest(_limit: uint256, _offset: uint256) \
  ...
  for index in range(0, MAX_ITERATIONS):
    ...
    col.append(helper._epochLatestByAddress(pool_data[1], pool_data[2]))
...
def epochsByAddress(_limit: uint256, _offset: uint256, _address: address) \
  ...
  @return Array for LpEpoch structs
  """
  return helper._epochsByAddress(_limit, _offset, _address)
...
def rewards(_limit: uint256, _offset: uint256, _venft_id: uint256) \
  ...
  for pindex in range(0, MAX_POOLS):
    ...
    pcol: DynArray[Reward, MAX_POOLS] = \
      helper._poolRewards(_venft_id, pool_data[1], pool_data[2])
...
def rewardsByAddress(_venft_id: uint256, _pool: address) \
  ...
  return helper._poolRewards(_venft_id, _pool, gauge_addr)
